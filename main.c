...
volatile uint8_t box_detected = 0;
volatile uint8_t box_mask = 0;
...
int main(void)
{
  ...
  MX_USART1_UART_Init();
  /* USER CODE BEGIN 2 */
  FSM_Init();				//Initializing FSM
  Color_Init(&htim3);		//Initializing color sensor

  // Starting PWM generation
  HAL_TIM_PWM_Start(&htim2, TIM_CHANNEL_1);   // PA0 = Motor A-1
  HAL_TIM_PWM_Start(&htim2, TIM_CHANNEL_2);   // PA1 = Motor A-2
  HAL_TIM_PWM_Start(&htim2, TIM_CHANNEL_3);   // PB10 = Motor B-1
  HAL_TIM_PWM_Start(&htim2, TIM_CHANNEL_4);   // PB11 = Motor B-2

  // Starting DMA to fill the raw_adc buffer
  HAL_ADC_Start_DMA(&hadc1, (uint32_t*)raw_adc, NUM_SENSORS);
  /* USER CODE END 2 */
